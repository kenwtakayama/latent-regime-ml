...
def _parse_args():
    p = argparse.ArgumentParser(description="Run experiments with a YAML config")
    p.add_argument("config", nargs="?", default="configs/medium.yaml",
                   help="path to config YAML file (default: configs/medium.yaml)")
    return p.parse_args()
